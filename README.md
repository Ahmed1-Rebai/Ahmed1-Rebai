<!-- The visuals in this profile are generated. Edit scripts/content.py, then run `python scripts/build.py`. -->

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/hero-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="assets/hero-light.svg">
    <img alt="Ahmed Rebai — AI Automation & RAG Systems Engineer. Data Engineering student in Sfax, Tunisia, building evidence-grounded AI, RAG systems and real-time data pipelines." src="assets/hero-dark.svg" width="100%">
  </picture>
</p>

<p align="center">
  <a href="https://www.linkedin.com/in/ahmed-rebai-18506938a/"><picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/btn-linkedin-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="assets/btn-linkedin-light.svg">
    <img alt="Connect on LinkedIn" src="assets/btn-linkedin-dark.svg" height="44">
  </picture></a>&nbsp;&nbsp;<a href="mailto:rebaiahmed244@gmail.com"><picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/btn-email-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="assets/btn-email-light.svg">
    <img alt="Email rebaiahmed244@gmail.com" src="assets/btn-email-dark.svg" height="44">
  </picture></a>
</p>

<p align="center">
  I'm a second-year <b>Data Engineering</b> student at the Faculty of Sciences of Sfax who builds AI the way good data
  pipelines are built: <b>every output traceable to its source, every change tested, every deploy automated.</b>
  In 2026 I shipped RAG, multi-agent and edge-AI systems across three internships and earned three
  Microsoft data certifications.
</p>

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/tiles-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="assets/tiles-light.svg">
    <img alt="At a glance: 3 Microsoft certifications (DP-900, DP-600, DP-700); 3 engineering internships in 2026; 421 automated tests shipped on a multi-agent AI platform; 0 hallucinated citations allowed by Postmortem Autopilot's CI gate." src="assets/tiles-dark.svg" width="100%">
  </picture>
</p>

<br>

<p>
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/h-experience-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="assets/h-experience-light.svg">
    <img alt="01 — Experience" src="assets/h-experience-dark.svg" width="100%">
  </picture>
</p>

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/experience-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="assets/experience-light.svg">
    <img alt="Experience: Software & Edge AI Engineer at CompiTechnology (Aug 2026, hybrid) — TinyML on Wear OS and Arduino nRF52, custom BLE/iBeacon protocol and ESP32 firmware, Laravel 12 REST API, real-time React dashboard. AI Automation Engineering Intern at Novagate Solutions (Jun–Jul 2026, remote) — RAG core, knowledge-graph memory, provider-agnostic LLM routing, security fixes. AI Systems Engineering Intern at DESLAB (Jul 2026, on-site) — multi-agent system with a hand-built ReACT loop, temporal knowledge graph retrieval, 421-test suite and GitLab CI/CD. Software Engineering Intern at Engineering & Consulting (Aug 2025, hybrid) — Nova Write CMS on ASP.NET Core MVC 8." src="assets/experience-dark.svg" width="100%">
  </picture>
</p>

<br>

<p>
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/h-work-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="assets/h-work-light.svg">
    <img alt="02 — Selected work" src="assets/h-work-dark.svg" width="100%">
  </picture>
</p>

<p align="center">
  <a href="https://github.com/Ahmed1-Rebai/Postmortem-Autopilot"><picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/postmortem-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="assets/postmortem-light.svg">
    <img alt="Postmortem Autopilot — evidence-grounded incident postmortems where every claim is traceable to a source event. Python, LangGraph, Neo4j, Docker, k3s, Helm, GitHub Actions." src="assets/postmortem-dark.svg" width="100%">
  </picture></a>
</p>

<p align="center">
  <a href="https://github.com/Ahmed1-Rebai/tradepulse"><picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/tradepulse-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="assets/tradepulse-light.svg">
    <img alt="TradePulse — real-time market data pipeline: Redpanda, VWAP, RSI-14 and anomaly detection into S3 and DynamoDB, with Terraform, Prometheus and Grafana." src="assets/tradepulse-dark.svg" width="100%">
  </picture></a>
</p>

<p align="center">
  <a href="https://github.com/Ahmed1-Rebai/Konsol"><picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/card-konsol-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="assets/card-konsol-light.svg">
    <img alt="Konsol — premium SSH client for mobile, built with Flutter." src="assets/card-konsol-dark.svg" width="49%">
  </picture></a>
  <a href="https://github.com/Ahmed1-Rebai/Tunisian_Footbal_DataWarehouse"><picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/card-football-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="assets/card-football-light.svg">
    <img alt="Tunisian Football Data Warehouse — Python ETL into a constellation schema for OLAP analytics and Power BI." src="assets/card-football-dark.svg" width="49%">
  </picture></a>
</p>

<p align="center">
  <a href="https://github.com/Ahmed1-Rebai/Champions_League"><picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/mini-champions_league-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="assets/mini-champions_league-light.svg">
    <img alt="Champions League ETL — Python, PostgreSQL, Power BI." src="assets/mini-champions_league-dark.svg" width="32.3%">
  </picture></a>
  <a href="https://github.com/Ahmed1-Rebai/RealTime-Fitness-Analyzer"><picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/mini-realtime-fitness-analyzer-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="assets/mini-realtime-fitness-analyzer-light.svg">
    <img alt="RealTime Fitness Analyzer — MediaPipe, OpenCV, Python." src="assets/mini-realtime-fitness-analyzer-dark.svg" width="32.3%">
  </picture></a>
  <a href="https://github.com/Ahmed1-Rebai/Opti-DataCenter-Tunisie"><picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/mini-opti-datacenter-tunisie-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="assets/mini-opti-datacenter-tunisie-light.svg">
    <img alt="Data Center MILP optimisation — PuLP, LINGO, Python." src="assets/mini-opti-datacenter-tunisie-dark.svg" width="32.3%">
  </picture></a>
  <a href="https://github.com/Ahmed1-Rebai/swag-labs-mobile-tests"><picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/mini-swag-labs-mobile-tests-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="assets/mini-swag-labs-mobile-tests-light.svg">
    <img alt="Mobile test suite — Appium, pytest, Python." src="assets/mini-swag-labs-mobile-tests-dark.svg" width="32.3%">
  </picture></a>
  <a href="https://github.com/Ahmed1-Rebai/NovaWrite"><picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/mini-novawrite-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="assets/mini-novawrite-light.svg">
    <img alt="Nova Write — ASP.NET Core MVC 8, EF Core, SQL Server." src="assets/mini-novawrite-dark.svg" width="32.3%">
  </picture></a>
  <a href="https://github.com/Ahmed1-Rebai/Super_Keyboard"><picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/mini-super_keyboard-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="assets/mini-super_keyboard-light.svg">
    <img alt="Super Keyboard — C++ and SFML rhythm game." src="assets/mini-super_keyboard-dark.svg" width="32.3%">
  </picture></a>
</p>

<br>

<p>
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/h-certs-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="assets/h-certs-light.svg">
    <img alt="03 — Certifications" src="assets/h-certs-dark.svg" width="100%">
  </picture>
</p>

<p align="center">
  <a href="https://learn.microsoft.com/en-us/credentials/certifications/fabric-data-engineer-associate/"><picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/cert-dp-700-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="assets/cert-dp-700-light.svg">
    <img alt="Microsoft Certified: Fabric Data Engineer Associate (DP-700), passed September 7, 2026." src="assets/cert-dp-700-dark.svg" width="32.3%">
  </picture></a>
  <a href="https://learn.microsoft.com/en-us/credentials/certifications/fabric-analytics-engineer-associate/"><picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/cert-dp-600-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="assets/cert-dp-600-light.svg">
    <img alt="Microsoft Certified: Fabric Analytics Engineer Associate (DP-600), passed August 28, 2026." src="assets/cert-dp-600-dark.svg" width="32.3%">
  </picture></a>
  <a href="https://learn.microsoft.com/en-us/credentials/certifications/azure-data-fundamentals/"><picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/cert-dp-900-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="assets/cert-dp-900-light.svg">
    <img alt="Microsoft Certified: Azure Data Fundamentals (DP-900), passed August 1, 2026." src="assets/cert-dp-900-dark.svg" width="32.3%">
  </picture></a>
</p>

<br>

<p>
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/h-stack-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="assets/h-stack-light.svg">
    <img alt="04 — Stack" src="assets/h-stack-dark.svg" width="100%">
  </picture>
</p>

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/stack-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="assets/stack-light.svg">
    <img alt="Stack — AI and LLM: RAG, multi-agent systems, ReACT loops, knowledge graphs, LangGraph, LangChain, OpenAI SDK, OpenRouter, n8n, PyTorch, TensorFlow, YOLO. Data: Microsoft Fabric, Kafka, Redpanda, PostgreSQL, SQL Server, Redis, Neo4j, pandas, Avro, Power BI. Cloud and Ops: AWS, Azure, Terraform, Docker, Kubernetes, k3s, Helm, GitHub Actions, GitLab CI/CD, Prometheus, Grafana. Edge and embedded: Edge Impulse, TFLite Micro, Arduino, ESP32, BLE/iBeacon, Wear OS, Android, C++/NDK/JNI. Languages: Python, TypeScript, JavaScript, Java, C, C++, PHP, Dart, C#, SQL. Web and QA: React, Next.js, Vue 3, Node.js, Express, FastAPI, Flask, Laravel 12, Inertia, ASP.NET Core, Flutter, pytest, Selenium, Appium." src="assets/stack-dark.svg" width="100%">
  </picture>
</p>

<br>

<p>
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/h-activity-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="assets/h-activity-light.svg">
    <img alt="05 — Activity" src="assets/h-activity-dark.svg" width="100%">
  </picture>
</p>

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/activity-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="assets/activity-light.svg">
    <img alt="GitHub activity over the last 12 months: contribution heatmap, streaks and top languages, refreshed daily by GitHub Actions." src="assets/activity-dark.svg" width="100%">
  </picture>
</p>

<br>

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/footer-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="assets/footer-light.svg">
    <img alt="Let's build AI that shows its work. rebaiahmed244@gmail.com · linkedin.com/in/ahmed-rebai-18506938a · Sfax, Tunisia" src="assets/footer-dark.svg" width="100%">
  </picture>
</p>
